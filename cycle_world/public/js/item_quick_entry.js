...
frappe.ui.form.ItemQuickEntryForm = frappe.ui.form.QuickEntryForm.extend({
	...
	preset_fields_for_template: function() {
		...
		this.dialog.get_field("is_stock_item").df.hidden = 1;
		this.dialog.get_field("opening_stock").df.hidden = 1;
		this.dialog.get_field("brand_name").refresh();
		this.dialog.get_field("variant_of").refresh();
		this.dialog.get_field("is_stock_item").refresh();
		this.dialog.get_field("opening_stock").refresh();
	...
	init_for_item_template_trigger: function() {
		...
		me.dialog.fields_dict["item_template"].df.onchange = () => {
			...
			this.dialog.get_field("brand_name").set_value(template);
			this.dialog.get_field("variant_of").set_value(template);
